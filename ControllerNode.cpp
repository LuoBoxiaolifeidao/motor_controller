...
void ControllerNode::init()
{
    ...
    jointFT_->syncReadBegin(ft_cfg_.ids.size(), 4, 100);
...
void ControllerNode::readPdo(Motor type)
{
    ...
    case Motor::FT: {
        ...
        if (jointFT_->syncReadPacketTx(ids, 3, SMS_STS_PRESENT_POSITION_L, 4) > 0) {
            u8 rxBuf[4];
            for (int k = 0; k < 3; ++k) {
...
void ControllerNode::writePdo(Motor type)
{
    ...
    case Motor::DM2C: {
        ...
        std::cout<<"target="<<targetPos_[static_cast<int>(Joint::Lift)]<<" err"<<err<<std::endl;
...
void ControllerNode::executeGripper(const common_comms::GripperCommandGoalConstPtr& goal)
{
    ...
    while (ros::ok()) {
        ...
        {
            std::lock_guard<std::mutex> lock(ft_mutex_);
 //           pos = jointFT_->ReadPos(gripper_cfg_.id);
              if(jointFT_->FeedBack(gripper_cfg_.id)!=-1)
                  pos = jointFT_->ReadPos(-1);        
        }
        std::cout<<"current pos = "<<pos<<"  target="<<target<<std::endl;
