...
IKResult solve_ik(const Vector& target_pos, const Rotation& target_rot,
  ...
  double tp_r, tp_p, tp_y; target_rot.GetRPY(tp_r, tp_p, tp_y);
  printf("[L-IK] target pos=[%.4f %.4f %.4f] rpy=[%.3f %.3f %.3f] init_w=[%.3f %.3f %.3f]\n",
         target_pos.x(), target_pos.y(), target_pos.z(), tp_r, tp_p, tp_y, q3, q4, q5);
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    printf("[L-IK] iter=%d  d0=%.3f t1=%.3f d_total=%.3f  w=[%.3f %.3f %.3f]  "
           "wp_z=%.3f A=%.3f B=%.3f\n",
           iter, d0, t1, d_total, q3, q4, q5, T_wp.p.z(), T_wp.p.x()+0.1275, T_wp.p.y()-0.0125);
    ...
    if (iter >= 1) {
      ...
      printf("[L-IK] iter=%d check: fk=[%.4f %.4f %.4f] pe=%.4f re=%.4f%s\n",
             iter, T_fk.p.x(), T_fk.p.y(), T_fk.p.z(), pe, re_ang,
             (pe < 0.01 && re_ang < 0.05) ? " OK" : "");
  ...
  printf("[L-IK] FINAL: fk=[%.4f %.4f %.4f] pe=%.4f re=%.4f  arm=[%.3f %.3f %.3f %.3f %.3f]  %s\n",
         T_fk.p.x(), T_fk.p.y(), T_fk.p.z(), result.pos_err, result.rot_err,
         d0, t1, dA, dB, dC, result.success ? "SUCCESS" : "FAIL");
  return result;
}
