...
class PickAndPlaceRight(Node):
    def __init__(self):
        super().__init__("pick_and_place_right")

        self.tcp_pub = self.create_publisher(Float64MultiArray, "/right_tcp", 10)
        self.finger_pub = self.create_publisher(Float64MultiArray, "/right_tool", 10)

        self.steps = [
            # (description, tcp_target, finger_target, attach, detach, delay_sec)
            ("move to cup",        [0.4, 0.3, 0.9, 0, 0, -1.57], None, False, False, 3.0),
            ...
            ("retract arm",        [0.35, 0.3, 0.9, 0, 0, -1.57], None, False, False, 3.0),
