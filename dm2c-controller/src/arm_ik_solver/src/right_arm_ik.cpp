...
static const Frame T_O2A(Rotation::RPY(-HALF_PI, 0, 0), Vector(0.0, 0.254, 0.033));
...
static const Frame T_O3(Rotation::RPY(-HALF_PI, 0, 0), Vector(0.0, 0.037, 0.066375));
static const Frame T_O4(Rotation::RPY(HALF_PI, 0, 0), Vector(0.0, 0.036375, 0.0565));
...
static std::array<double, 3> solve_wrist_from_R(const Rotation& R) {
  ...
  if (std::abs(s4) > 1e-8) {
    ...
    q3 = 0.0;
    q5 = (c4 > 0) ? std::atan2(-R(1, 0), -R(0, 0)) : std::atan2(R(1, 0), R(0, 0));
  }
...
static void solve_arm_from_wp(const Vector& P_wp, double& d0, double& t1, double& d_total) {
  ...
  d_total = clamp(L - 0.346, JOINT_LOWER[2], JOINT_UPPER[2] + JOINT_UPPER[3] + JOINT_UPPER[4]);
...
static void split_arm(double d_total, double& dA, double& dB, double& dC) {
  ...
  dA = clamp(d_total, JOINT_LOWER[2], maxA); d_total -= dA;
  dB = clamp(d_total, JOINT_LOWER[3], maxB); d_total -= dB;
...
bool load_chain_from_urdf(const std::string& urdf_xml, KDL::Chain& chain) {
  ...
  return tree.getChain("base_link", "r_wrist_r_Link", chain);
}
