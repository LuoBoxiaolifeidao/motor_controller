...
static std::array<double, 3> solve_wrist_from_R_alt(const Rotation& R) {
  double c4 = clamp(R(2, 2), -1.0, 1.0);
  double q4 = std::acos(c4);
  double s4 = std::sin(q4);
  ...
  double q3, q5;
  if (std::abs(s4) > 1e-8) {
    q3 = std::atan2(-R(1, 2), -R(0, 2)) + M_PI;
    q5 = std::atan2(-R(2, 1), R(2, 0)) + M_PI;
    q4 = -q4;
  } else {
    q3 = 0.0;
    q5 = (c4 > 0) ? std::atan2(-R(1, 0), -R(0, 0))
...
IKResult solve_ik(const Vector& target_pos, const Rotation& target_rot,
  ...
  for (int iter = 0; iter < 5; ++iter) {
    ...
    auto wq_a = solve_wrist_from_R(R_wrist_target);
    auto wq_b = solve_wrist_from_R_alt(R_wrist_target);

    auto eval_wrist = [&](const std::array<double,3>& wq,
                          std::array<double,8>& q8, double& pe, double& re_ang) {
      double dA, dB, dC; split_arm(d_total, dA, dB, dC);
      q8 = {d0, t1, dA, dB, dC, wq[0], wq[1], wq[2]};
      Frame T = fk(q8);
      pe = (target_pos - T.p).Norm();
      Rotation re_t = target_rot * T.M.Inverse();
      double trace = re_t(0,0) + re_t(1,1) + re_t(2,2);
      re_ang = std::acos(clamp((trace - 1.0) / 2.0, -1.0, 1.0));
    };

    std::array<double,8> qa, qb;
    double pea, rea, peb, reb;
    eval_wrist(wq_a, qa, pea, rea);
    eval_wrist(wq_b, qb, peb, reb);

    if (pea + 0.1 * rea <= peb + 0.1 * reb) {
      q3 = wq_a[0]; q4 = wq_a[1]; q5 = wq_a[2];
      if (pea < 0.01 && rea < 0.05) {
        result.q8 = qa; result.pos_err = pea; result.rot_err = rea;
        result.success = true; return result;
      }
    } else {
      q3 = wq_b[0]; q4 = wq_b[1]; q5 = wq_b[2];
      if (peb < 0.01 && reb < 0.05) {
        result.q8 = qb; result.pos_err = peb; result.rot_err = reb;
        result.success = true; return result;
      }
    }
  }
  ...
  // final pass
  {
    auto eval = [&](const std::array<double,3>& wq,
                    std::array<double,8>& q_out, double& pe_out, double& re_out) {
      double dA, dB, dC; split_arm(d_total, dA, dB, dC);
      q_out = {d0, t1, dA, dB, dC, wq[0], wq[1], wq[2]};
      Frame T = fk(q_out);
      pe_out = (target_pos - T.p).Norm();
      Rotation re_t = target_rot * T.M.Inverse();
      double trace = re_t(0,0) + re_t(1,1) + re_t(2,2);
      re_out = std::acos(clamp((trace - 1.0) / 2.0, -1.0, 1.0));
    };

    std::array<double,8> qa, qb;
    double pea, rea, peb, reb;
    eval(wq_a, qa, pea, rea);
    eval(wq_b, qb, peb, reb);
